...
var Vm = new Vue({
    ...
    data: {
        consoleData: [], // 控制台日志
        messageData: [], // (调试)消息记录  切换消息列表的
        chatData: [],
        instance: WebSocket, // ws instance
        address: 'ws://127.0.0.1:8886/ws', // 链接地址
        username: "daozhu",
        alert: {
            class: 'success',
            state: false,
        ...
        tagSwitchState: {
            navSwitch: {
                messageListShowState: 'show',
                friendListShowState: '',
                settingListShowState: ''
            },
            memberListOpenSwitch: {
                openStats: ''
            }
        }
    },
    ...
    methods: {
        ...
        autoWsConnect: function () {
            try {
                if (this.connected === false){
                    var url = this.address + "?username=" + this.username;
                    
                    localStorage.setItem('address', this.address);
                    localStorage.setItem('username', this.username);
                    var wsInstance = new WebSocket(url);
                    var _this      = Vm
        ...
        release: function(controller, action, parmas){
            ...
            try {
                ...
                Vm.writeAlert('danger', '消息发送失败 原因请查看控制台');
                throw err;
            }
        },
        getOnlineList: function(){
            this.release('user', 'list', {username: ""});
        },
        tagSwitch: function(tagName){
            // TODO 函数自动循环设置  三级或者无限递归
            // tagName = tagSwitchState.navSwitch.messageListShowState 本身代表多级，取倒数第二个下的所有tag，自动切换
            switch (tagName) {
                case "messageList":
                    this.tagSwitchState.navSwitch.messageListShowState = "show";
                    this.tagSwitchState.navSwitch.friendListShowState = "";
                    this.tagSwitchState.navSwitch.settingListShowState = "";
                    break;
                case "friendList":
                    this.tagSwitchState.navSwitch.messageListShowState = "";
                    this.tagSwitchState.navSwitch.friendListShowState = "show";
                    this.tagSwitchState.navSwitch.settingListShowState = "";
                    break;
                case "settingList":
                    this.tagSwitchState.navSwitch.messageListShowState = "";
                    this.tagSwitchState.navSwitch.friendListShowState = "";
                    this.tagSwitchState.navSwitch.settingListShowState = "show";
                    break;
            }
        },
        openMembersList: function(){
            if (this.tagSwitchState.memberListOpenSwitch.openStats) {
                this.tagSwitchState.memberListOpenSwitch.openStats = '';
            } else {
                this.tagSwitchState.memberListOpenSwitch.openStats = 'direct-chat-contacts-open';
            }
        }
    }
});