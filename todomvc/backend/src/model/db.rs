...
pub async fn init_db() -> Result<Db, sqlx::Error> {
	// -- Create the db with PG_ROOT (dev only)
	{
		...
		pexec(&root_db, SQL_RECREATE).await?; // 只执行 00-recreate-db.sql
	}

	// -- Run the app sql files (执行除了 recreate 之外的其他 sql)
...
async fn new_db_pool(host: &str, db: &str, user: &str, pwd: &str, max_con: u32) -> Result<Db, sqlx::Error> {
	...
	PgPoolOptions::new()
		...
		.connect_timeout(Duration::from_millis(500)) // Needs to find replacement
		.connect(&con_string)
		.await
}
